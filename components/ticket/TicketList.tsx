...
export function TicketList({ apiBase, status }: TicketListProps) {
  ...
  if (loading) return (
    <div className="p-4 sm:p-6 md:p-8 flex items-center justify-center">
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <span className="text-sm sm:text-base text-gray-600 font-medium">{t("loading.tickets")}</span>
      </div>
    </div>
  );
  ...
  if (error) return (
    <div className="p-4 sm:p-6 md:p-8 text-center">
      <div className="inline-flex items-center gap-1 sm:gap-2 text-red-600 bg-red-50 px-3 sm:px-4 py-2 sm:py-3 rounded-lg sm:rounded-xl border border-red-200">
        ...
        <span className="font-medium text-sm sm:text-base">{t("error")}: {error}</span>
      </div>
    </div>
  );
  ...
  if (!items.length) return (
    <div className="p-6 sm:p-8 md:p-12 text-center">
      ...
      <p className="text-gray-600 font-medium text-sm sm:text-base">{t("no.tickets.found")}</p>
      <p className="text-gray-500 text-xs sm:text-sm mt-1">{t("no.tickets.moment", {status: status.toLowerCase()})}</p>
    </div>
  );
