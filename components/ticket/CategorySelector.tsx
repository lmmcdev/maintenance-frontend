...
export function CategorySelector({
  ...
  useEffect(() => {
    const currentValue = typeof t.subcategory === 'object' && t.subcategory?.displayName 
  ...
  const handleCategorySelect = async (categoryName: string) => {
    ...
    try {
      ...
      let subcategoryToAssign = null;
      
      cats.forEach(category => {
        const foundSubcat = category.subcats.find(sc => sc.displayName === categoryName);
        if (foundSubcat) {
          subcategoryToAssign = foundSubcat;
        }
      });

      if (subcategoryToAssign) {
        await patchTicket(apiBase, t.id, { 
          subcategory: { 
            name: subcategoryToAssign.name, 
            displayName: subcategoryToAssign.displayName 
          } 
        });
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
      <span className="text-gray-600 text-xs sm:text-sm min-w-[48px] sm:min-w-[64px] font-semibold">{translate("category")}</span>
      <div className="flex-1 relative" ref={containerRef}>
        ...
        >
          <span>
            {selectedName || translate("category.select")}
          </span>
          <svg 
            width="20" 
            height="20" 
            ...
            className={`transition-transform duration-200 ${
              ...
              isOpen && dropdownPosition === 'down' ? 'rotate-180' : ''
        ...
        {isOpen && !busy && (
          <div className={`absolute z-[9999] w-full bg-white border border-gray-200 rounded-lg sm:rounded-xl shadow-2xl max-h-60 overflow-hidden ${
            ...
            <div className="max-h-40 overflow-y-auto">
              {filteredCategories.map((category) => (
                ...
                >
                  <span className="text-xs sm:text-sm text-gray-700 flex-1">{category}</span>
                  {selectedName === category && (
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="text-[#00A1FF] ml-2">
